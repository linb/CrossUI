    ...
    Instance:{
        ...
        applyCellFormula:function(cellTo, dirtyMark, triggerEvent){
            return this.each(function(prf){
                var tg=prf.box,  formula, needUpdate,t2,cellsMap={},coo,tcell,
                    prop = prf.properties,
                    rows=prop.rows,
                    // only for first level
        ...
        */
        _insertRowsToDom:function(profile, arr, pid, base, before,temp,mode){
            ...
            xui.arr.each(arr,function(o){
                ...
                xui.arr.each(o.cells,function(v){
                    v.width=v._col._colWidth;
                });
            });

            //build dom
            var nodes21 = profile._buildItems(pid&&profile.rowMap[pid]._region==1?'rows11':'rows21', arr),
                nodes22 = profile._buildItems(pid&&profile.rowMap[pid]._region==1?'rows12':'rows22', arr);
            ...
            if(profile.afterInsertRowsToDom){
                profile.boxing().afterInsertRowsToDom(profile,xui.toArr(arguments),nodes21,nodes22);
            }
            if(mode!="domOnly")
                profile.box._adjustBody(profile,'addrow');
        },
        // for dynamic renderer
        scrollToRow:function(id, callback){
            var ns=this, prf=ns.get(0), prop=prf.properties, dftRowH=prf.$px(prop.rowHeight),top=0,
            findRowTop = function(row_id, rows, propRowH, dftRowH, rowHEdge){
                for(var i=0,l=rows.length;i<l;i++){
                  var row=rows[i];
                  if(row.id === row_id){
                    return false;
                  }
                  top += row.hidden? 0 : ( ( xui.isSet(row._rowHeight) ? propRowH ==  row._rowHeight ? dftRowH : prf.$px(row._rowHeight) : xui.isSet(row.height) ? prf.$px(row.height) : dftRowH) + rowHEdge );
                  if(xui.isArr(row.sub)){
                      return findRowTop(row_id, row.sub,propRowH, dftRowH, rowHEdge);
                  }
                }
            };
            findRowTop(id, prop.rows, prop.rowHeight, dftRowH, xui.get(prop.nodeEdges,["rowH"])||0 );
            prf.getSubNode("SCROLL22").get(0).scrollTop = top;
            ns.adjustPage("custom", false, callback);
        },
        adjustPage:function(trigger, force, callback){
          var ns=this, prf=ns.get(0), renderViewSize=prf.properties.renderViewSize;
          if(!renderViewSize || prf._ignoreScrollTrigger){
              xui.tryF(callback);
              return;
          }

          var clearSubRange = function(row){
              if(row.sub && row.sub.length){
                  delete row._renderRange;
                  delete row._o_renderRange;
                  for(var i=0,l=row.sub.length;i<l;i++){
                      clearSubRange(row.sub[i]);
                  }
              }
          },
          ...
          adjustRowsDisplay = function(prf, row, subRows21, subRows22, renderFrom, pid){
              if( !row.hasOwnProperty("_renderRange") || row._renderRange[0] == -1 )return;

              // set 3 height
              var nodes = xui().merge(subRows21).merge(subRows22);
              nodes.prev().height(prf.$forceu(row._space_before||0));
              nodes.next().height(prf.$forceu(row._space_after||0));
              nodes.height( prf.$forceu(row._space_sub) );

              var keep_from = Math.max(row._o_renderRange[0], row._renderRange[0]),
                  keep_to = Math.min(row._o_renderRange[1], row._renderRange[1]);
              ...
              var subRows = pid ? row.sub : prf.properties.rows;

              // fill new rows
              var bs=[],as=[];
              for(var i=row._renderRange[0], l=row._renderRange[1];i<=l;i++){
                  if(i < keep_from)bs.push(subRows[i]);
                  else if(i > keep_to)as.push(subRows[i]);
              }
              if(bs.length)
                  prf.boxing().insertRows(bs, pid, null, true, true, false, "domOnly");
              if(as.length)
                  prf.boxing().insertRows(as, pid, null, false, true, false, "domOnly");

              // for sub rows
              if(!pid || (subRows && subRows.length && row._checked && row._inited)){
                  if(row._renderRange[0] !=-1 ){
                    for(var i=row._renderRange[0], l=row._renderRange[1], r;i<=l;i++){
                        r = subRows[i];
                        if(r.sub && r.sub.length && r._serialId){
                            adjustRowsDisplay(prf, r, prf.getSubNode("SUB1", r._serialId), prf.getSubNode("SUB2", r._serialId), renderFrom, r.id);
                        }
                    }
                  }
              }
          ...
          if(force) prf._$viewHeight=-1;

          xui.resetRun('tg_adjust_page:' + prf.$xid, function(){
              var scrollNode=prf.getSubNode('SCROLL22'), top=scrollNode.scrollTop(),height=Math.max(100, scrollNode.height());
              if(height){
                  if(prf._$viewTop !== top || prf._$viewHeight !== height){
                      if(prf.beforeAdjustPage && false===ns.beforeAdjustPage(prf, trigger)){
                          xui.tryF(callback);
                          return;
                      }

                      var prop=prf.properties, dftRowH=prf.$px(prop.rowHeight),
                          // renderViewSize min is 100%
                          renderFrom = Math.floor( Math.max(0, top - Math.min(1, renderViewSize-1)/2*height )),
                          renderTo = Math.ceil(top + height + Math.min(1, renderViewSize-1)/2*height);
                      ...
                      prf._space_sub = prf._space_main;

                      // avoid tigger other call
                      prf._ignoreScrollTrigger = 1;

                      adjustRowsDisplay(prf, prf, prf.getSubNode("ROWS21"), prf.getSubNode("ROWS22"), renderFrom, null);

                      prf.boxing().setUIValue(prf.boxing().getUIValue(), true,null,'page');
                      ...
                      xui.tryF(callback);
                      if(xui.isFun(prf._$afterAdjustPage))prf._$afterAdjustPage.call(ns, prf, trigger, top, height, renderFrom, renderTo);
        ...
        */
        openToNode:function(id, triggerEvent, stopanim, intoView, callback){
            return this.each(function(profile){
                ...
                fun.call(null, profile.properties.rows, id);
                if(res){
                    a.reverse();
                    xui.arr.each(a,function(o,i){
                        if(o.sub){
                            profile.boxing().toggleRow(o.id,true,false, stopanim);
                        }
                    });
                    if(intoView){
                        profile.boxing().scrollToRow(id, callback);
                    }else{
                        xui.tryF(callback);
                    }
                }
        ...
        getRowbyRowId:function(rowId, type, splitMixColumn){
            var profile=this.get(0),v=profile.rowMap2,rows=profile.properties.rows;
            ...
            if((v=profile.queryItems(rows, function(v){
                return v.id==rowId;
            }, 1,1))&&v.length)
        ...
        //pid,base are id
        insertRows:function(arr, pid/*true: the current item*/, base/*true: the current item*/, before, ignoreMixColumn, ensureHotRow, mode){
            var ns=this,
                rows, v,
                c=ns.constructor,
                profile=ns.get(0),
                prop=profile.properties;
            if(mode!="domOnly" && profile.beforeInsertRows && false === ns.beforeInsertRows(profile,xui.toArr(arguments))){
              return false;
            }

            if(xui.isHash(arr))arr=[arr];
            if(arr && xui.isArr(arr) && arr.length>0){
                var row_m=profile.rowMap2,
                    ro=prop.rowOptions,
                ...
                // prepare first
                c._prepareItems(profile, arr, pid, null, "memoryOnly");
                ...
                //1 add to dom
                if(mode!="memoryOnly"){
                    if(profile.renderId && (!prop.renderViewSize || !pid || (pid && profile.boxing().getSubNodes(["ROW1","ROW2"], pid).get(0)) )){
                        // if insert to root, or the parent node is inited
                        if(!xui.isSet(pid) || k._inited){
                            //prepareData(add links)
                            temp=[];
                            // prepare again, for UI data
                            rows = c._prepareItems(profile, arr, pid, temp);
                            ns._insertRowsToDom(profile, rows, pid, base, before,temp,mode);
                ...
                xui.arr.each(arr,function(o){
                    if(o.sub){
                        o.open=false;
                        // only for not _inited row
                        if(!o._inited && false===profile.box.getCellOption(profile, o, "iniFold"))
                            profile.boxing()._toggleRows([o],true,false,true);
                    }
                });
                // 4 check ext nodes
                if(profile.renderId){
                    profile.box._asyDecorate(profile, 'inserRows');
                }
                // 5 hot row
                if(ensureHotRow!==false && profile.renderId){
                    profile.box.__ensurehotrow(profile,ensureHotRow);
                }
                // 6 try to hide ui-no-children row
                ...
                // 7 event
                if(mode!="domOnly" && profile.afterInsertRows){
                    ns.afterInsertRows(profile,xui.toArr(arguments),rows);
                }
                // 8 try to pagination
                ns.adjustPage("afterInsertRows", true);
            }
        ...
        removeRows:function(ids/*default is the current*/,ensureHotRow){
            ...
            var p=profile.properties,
                ...
                nodes=[],v,count=0;
            if(profile.beforeRemoveRows && false===self.beforeRemoveRows(profile,xui.toArr(arguments))){
              return false;
            }
            //get array
            ...
            if(count>0){
                if(profile.afterRemoveRows){
                    self.afterRemoveRows(profile,xui.toArr(arguments));
                }
                ...
                if(profile.afterRemoveRowsFromDom){
                    profile.boxing().afterRemoveRowsFromDom(profile,xui.toArr(arguments));
                }

                profile.box._asyDecorate(profile, 'removeRows');
                profile.box._adjustBody(profile,'delrow');

                if(ensureHotRow!==false && profile.renderId){
                    profile.box.__ensurehotrow(profile,ensureHotRow);
                }
                self.adjustPage("afterRemoveRows", true);
        ...
        getRowMap:function(rowId, withRowVars){
            var prf=this.get(0),ins=prf.boxing(),p=prf.properties,t;
        ...
        getSubNode:function(key, itemId, tag){
            if(key.indexOf("CELL")===0 && /\d+-\d+/.test(itemId)){
                var a = itemId.split("-");
                a = this.getCellbyRowCol(parseInt(a[0],10), parseInt(a[1],10));
                itemId = a._serialId;
            }
            return xui.UI.prototype.getSubNode.apply(this, [key, itemId, tag]);
        },
        ...
        adjustEditor:function(adjustFun){
            var ns=this,prf=this.get(0),
                cb=prf._$cache.hasOwnProperty('_root_cb') ? prf._$cache._root_cb : (prf._$cache._root_cb = prf.getRoot().contentBox()?1:0);
        ...
        adjustRelWith:function(){
            return this.each(function(profile){
                if(profile.renderId && profile.box._hasRelW(profile))
    ...
    Static:{
        ...
        Templates:{
                        ...
                        TRBODY:{
                            ...
                            TDBODY21:{
                                ...
                                SCROLL21:{
                                    $order:1,
                                    tagName:'div',
                                    className:'xui-forcescroll {_columnfreezed}',
                                    BODY21:{
                                        tagName:'div',
                                        ROWS21_B:{
                                            $order:1,
            ...
            $submap : {
                ...
                header:{
                    HCELL:{
                        ...
                        HCELLA:{
                            ...
                            HCELLCAPTION:{
                                $order:5,
                                className:'xui-v-node {caption_class}',
                                text:"{caption}"
                            },
                            SORT:{
                                className:'xuifont',
                ...
                rows21:function(profile,template,v,tag,result,index){
                    profile.rowMap[v._serialId]._region=profile.rowMap[v._serialId]._region||2;
                    // keep realtag for real data
                    xui.UI.$doTemplate(profile,template,v, "row1", result, index,'rows1');
                },
                ...
                'rows1._handler_cell1':{
                    FCELL:{
                        ...
                        CELLA:{
                            ...
                            ROWNUM:{
                                $order:4,
                                className:'xui-ui-readonly {number_class}',
                    ...
                    CELL:{
                        ...
                        CELLA:{
                            ...
                            CELLCAPTION:{
                                $order:6,
                                style:'{color}',
                                className:'xui-v-node xui-treegrid-fcellcaption {caption_class}',
                                text:"{_caption}"
                            }
                        }
                    }
                ...
                'rows.cells.checkbox':{
                    CELL:{
                        ...
                        CELLA:{
                            ...
                            tabindex: '{_tabindex}',
                            CHECKBOX:{
                                className:'xuifont xui-uicmd-check {checkbox_class}',
                    ...
                    CELL:{
                        ...
                        CELLA:{
                            ...
                            PROGRESS:{
                                $order:2,
                                tagName:'div',
                                className:'xui-v-wrapper xui-uibar {progress_class}',
                                style:'width:{progress};{progress_style}',
                                CELLCAPTION:{
                                    className:'xui-v-node {caption_class}',
                                    text:"{_caption}"
                                }
        ...
        Appearances:{
           ...
           'CELLS1-hover, CELLS2-hover':{
            ...
            'ROWS21_B, ROWS21_A, ROWS22_B, ROWS22_A, SUB1_B, SUB1_A, SUB2_B, SUB2_A':{
                position:'relative'
            },
            ROWTOGGLE:{
                padding:'0 .334em 0 0'
        ...
        Behaviors:{
            ...
            SCROLL22:{
                ...
                onSize:function(profile, e, src){
                    var node=xui.use(src), h = node.height();
                    if(profile.$ssh != h){
                      profile.$ssh = h;
                      profile.boxing().adjustPage("onSize");
                    }
                }
            },
            SCROLL21:{
            ...
            HCELLA:{
                onClick:function(profile, e, src){
                    ...
                    fun = me.fun||(me.fun = function(profile, subNode, index, type, sortby,order,lastrownode1,lastrownode2){
                        ...
                        //sort memory array
                        //sort dom node
                        var b = subNode._inited, bak=xui.copy(rows);
                        if(b){
                    ...
                    profile.box._asyDecorate(profile, 'sort');
                ...
                beforeMouseover:function(profile, e, src){
                    ...
                    var nn=xui.use(psrc).get(0), left=nn.offsetLeft + (col._region==2?profile._leftregionw:0), top=nn.offsetTop+(nn.offsetParent?nn.offsetHeight:0);
            ...
            CELLA:{
                ...
                onKeydown:function(profile, e, src){
                    ...
                    var p = profile.properties,
                        ...
                        hasBody12 = !!body12.isDisplayed() ,
                        hasBody21 = !!body21.isDisplayed() ,
                        hasBody22 = !!body22.isDisplayed() ,
                    ...
                    if(value)
                        nodes.each(function(o){
                            if((row=map[ns.getSubId(o.id)])&&row.id!=ns.box._temprowid){
                                l=row._layer;
                                pid=row._pid;
                                // get order in parent sub
                                pRow = pid?ns.rowMap[pid]:ns.properties.rows;
                                i = xui.arr.subIndexOf(pRow.sub || pRow, "id", row.id) + 1;
                                row._autoNumber=f((pRow._autoNumber?(pRow._autoNumber+"."):"") + i);
        ...
        LayoutTrigger:function(){
            var ns=this, box=ns.box, prop=ns.properties;
        ...
        _asyDecorate:function(profile, trigger){
            var prop=profile.properties,b=profile.boxing(),id=profile.$xid;
            if(prop.altRowsBg && profile.rowMap)xui.resetRun(id+"1",function(){if(profile.rowMap)b.setAltRowsBg(true,true)});
            if(prop.rowNumbered && profile.rowMap)xui.resetRun(id+"2",function(){if(profile.rowMap)b.setRowNumbered(true,true)});
        },
        _setRowHanderW:function(profile, flag){
            ...
            if(xui.isFinite(flag) || profile.$isEm(flag)|| profile.$isPx(flag))
                ...
                profile.getSubNode('FCELLCAPTION',true).each(function(o){
                    if((t=o.parentNode).offsetParent && xui.Dom.getStyle(t,'overflow')!='visible')
        ...
        _prepareData:function(profile){
            ...
            if(profile.onInitList){
                var ins = profile.boxing(),
                rst = profile.boxing().onInitList(profile, function(rst){
                    if(xui.isArr(rst)){
                        ins.setRows(rst);
                    }else{
                        if(xui.isArr(rst.grpCols)){
                            prop.grpCols = rst.grpCols;
                        }
                        if(xui.isArr(rst.columns||rst.header)){
                ...
                if(rst){
                    if(xui.isArr(rst)){
                        ...
                        if(xui.isArr(rst.columns||rst.header)){
                            prop.header = rst.columns||rst.header;
                        }
                        if(xui.isArr(rst.rows)){
                            prop.rows = rst.rows;
                        }
            ...
            data.rows11 = data.rows12 = prop.freezedRow ? this._prepareItems(profile, prop.rows.slice(0, prop.freezedRow)) : [];
            data.rows21 = data.rows22 = this._prepareItems(profile, prop.rows.slice(prop.freezedRow||0), null, null, prop.renderViewSize?"memoryOnly":"all");
        ...
        _renderCell:function(profile, cell, uicell, node,options){
            ...
            switch(type){
                ...
                default:
                    cell.value=cell.hasOwnProperty("value")?cell.value:"";
                    caption=xui.adjustRes(unit(xui.isSet(capOut)?capOut:ren(profile,cell,uicell,f8),cell));
                    if(node)node.html((caption===null||caption===undefined)?cell.value:caption,false);
            }
        ...
        _prepareItems:function(profile, arr, pid ,temparr, mode){
            var self=this,
                prop=profile.properties,
            ...
            for(var i=0,l=arr.length;i<l;i++){
                ...
                if(mode!="memoryOnly"){
                    t={id: row.id};
                    t.rowCls = ""
                    if(row.disabled) t.rowCls += ' xui-uicell-disabled';
                    if(row.readonly) t.rowCls += ' xui-ui-readonly';
                    if(row.group){
                        t.group=1;
                        t.rowCls += ' ' + profile.getClass('CELLS1','-group') + " xui-uiborder-r xui-uiborder-light";
                    }

                    if(row.hidden) t.rowDisplay=NONE;
        ...
        _setSub:function(profile, item, flag, recursive, stopanim, cb){
            var id=profile.domId,
                ins=profile.boxing(),
                prop=profile.properties;
            if(!prop.treeMode || prop.treeMode=="none")return;
            var itemId = profile.rowMap2[item.id],
            ...
            if(!flag){
                if(item._checked){
                    ...
                    var onend=function(){
                        ...
                        ins.adjustPage("afterFold", true);

                        xui.resetRun(id, function(cb){
                            profile.box._asyDecorate(profile, 'expand');
                            //clear rows cache
                            delete profile.$allrowscache1;
                            delete profile.$allrowscache2;
                ...
                if(!item._checked){
                    ...
                    var onend=function(empty,t){
                       ...
                       xui.resetRun(id, function(cb){
                            profile.box._asyDecorate(profile, 'fold');
                            //clear rows cache
                            delete profile.$allrowscache1;
                            delete profile.$allrowscache2;
                        ...
                        if(!empty && !item._inited){
                            ...
                            if(sub){
                                if(typeof sub=='string'){
                                    ...
                                    b.insertRows(sub, item.id, null, true, true, false, item._region!=1&&p.renderViewSize?"memoryOnly":"all");
                                    // for []
        ...
        _adjustBody:function(profile, trigger, callback){
            ...
            //xui.UI.$freezeNode(profile, 'adjustbody:' + trigger);
            xui.resetRun(profile.$xid+'4',function(){
                ...
                if(profile.renderId || !profile.destroyed){
                    ...
                    xui.idleRun(function(){
                        ...
                        var b21e=b21.isEmpty(),
                            ...
                            s12e=s12.isEmpty(),
                            fr=prop.freezedRow,
                            pb=!s21e?((s22.isScrollBarShowed('x')?xui.Dom.getScrollBarSize():0) ):0,
                            st=!s21e?s22.scrollTop():0,
                            sl=(fr&&!s12e)?s22.scrollLeft():0,
                            pr=(fr&&!b12e)?((s22.isScrollBarShowed('y')?xui.Dom.getScrollBarSize():0)):0

                        if(pb&&!b21e)b21.css('padding-bottom', pb+"px");
                        if(pr&&fr&&!b12e)b12.css('padding-right', pr+"px");
                        if(st&&!s21e)s21.scrollTop(st);
                        if(sl&&fr&&!s12e)s12.scrollLeft(sl);
                    });
                    ...
                    var overflowX="auto";
                    if(profile.box._hasRelW(profile)){
                        overflowX=profile.box._adjustRelWith(profile);
                    }
                    ...
                    var body12=profile.getSubNode('ROWS12'),
                        ...
                        scroll=profile.getSubNode('SCROLL22'),
                        t,l,last1,last2,ww,bw=0,hiw,bodyw;
                    ...
                    if(body22.get(0).clientHeight){
                        if(header.get(0).clientHeight){
                            ...
                            ww=last1 && last1.offsetParent?(last1.offsetWidth+last1.offsetLeft):0;
                    ...
                    if(last2||last1){
                        ...
                        var prop = profile.properties,hd=prop.header,
                        //defult
                        w = 0;
                ...
                //xui.UI.$unfreezeNode(profile,'adjustbody:' + trigger);
            });
            // formula
        ...
        _hasRelW:function(profile){
            for(var i in profile.colMap){
                if(profile.colMap[i].flexSize){
                    return true;
                }
            }
            return false;
        ...
        _adjustRelWith:function(profile, force){
            ...
            if(!profile._$cache.hasOwnProperty('_col_border_w')){
              profile.getSubNodes('HCELL',true).each(function(hc){
                  if(hc.clientHeight){
                      profile._$cache._col_border_w = (hc.offsetParent?hc.offsetWidth:0)- profile.$px(hc.style.width);
                      return false;
                  }
              });
            }
            ...
            if(!profile._$cache.hasOwnProperty("__lcellW")){
              var hc = profile.getSubNode("LHCELL").get(0),
                lcellw = xui.get(hc,["firstChild","firstChild"]) || xui.get(hc,["lastChild","firstChild"]) ? (hc.offsetParent?hc.offsetWidth:0) : 0;
              profile.getSubNodes('LCELL',true).each(function(hc){
                  if(xui.get(hc,["firstChild","firstChild"]) || xui.get(hc,["lastChild","firstChild"])){
                      ...
                      lcellw=Math.max(lcellw, (hc.offsetParent?hc.offsetWidth:0));
                      return;
                  }
              });
