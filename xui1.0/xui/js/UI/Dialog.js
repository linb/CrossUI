...
if(xui.browser.ie){
    ...
    xui.Dom.byId(id).innerHTML=_()+"";
        ...
        alert:function(title, content, onClose, btnCap, left, top, parent, subId, noCache){
            ...
            if(!_.isSet(left))left=((parent.get(0)==xui('body').get(0)?xui.win:parent).width()-size.width)/2;
            if(!_.isSet(top))top=((parent.get(0)==xui('body').get(0)?xui.win:parent).height()-size.height)/2;
        ...
        prompt:function(title, caption, content, onYes, onNo, btnCapYes, btnCapNo, left, top, parent, subId, noCache){
            ...
            if(parent && parent["xui.UI"])parent=parent.getContainer(subId);
            if(!_.isSet(parent))parent=xui('body');
            if(!_.isSet(left))left=((parent.get(0)==xui('body').get(0)?xui.win:parent).width()-dialog.getWidth())/2;
            if(!_.isSet(top))top=((parent.get(0)==xui('body').get(0)?xui.win:parent).height()-dialog.getHeight())/2;
